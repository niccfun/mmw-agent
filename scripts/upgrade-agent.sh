#!/usr/bin/env bash
# 手动升级 mmw-agent �?GitHub release(默认 latest,可指定版本如 v0.1.4)�?
#
# 适用场景:UI "升级"按钮卡住、agent 进程没换、需要绕过卡�?handler 强制刷新�?
#
# 用法:
#   bash upgrade-agent.sh              # 升级�?GitHub latest
#   bash upgrade-agent.sh v0.1.4       # 升级到指�?tag
#
# 兼容:
#   - systemd (Debian/Ubuntu/CentOS �? �?systemctl restart mmw-agent
#   - OpenRC (Alpine LXC �?            �?rc-service mmw-agent restart
#   - 都不在则�?supervise-daemon / �?nohup 启动(打印提示由用户接�?
#
# 失败兜底:
#   - 下载失败 �?退�?不动现有 binary
#   - 替换前自动备份到 /usr/local/bin/mmw-agent.bak-<timestamp>,启动失败可手动回�?
#
set -euo pipefail
...
# 3. 下载到临时位�?--max-time 防止网络卡死无限�?
# 镜像�?�?GitHub 优先,失败再自动降级到 CDN 代理。纯 v6 机器直连 github �?network is unreachable"
# (release binary 重定向到�?AAAA �?objects.githubusercontent.com),会快速失败后降级�?
...
[ "$download_ok" = "1" ] || err "所有镜像均下载失败(GitHub + ghproxy + gh-proxy 全部不可�?"
SIZE=$(du -h "$TMP" | cut -f1)
NEW_MD5=$(md5sum "$TMP" | awk '{print $1}')
log "下载完成: $SIZE, md5=$NEW_MD5"

# 3b. 签名校验:下载同名 .sig,用【已装】agent 的内嵌公钥验�?私钥离线,主控/本仓库都没有)�?
#     - rc=0  通过 �?继续
#     - rc=1  新版 agent 明确判定签名不匹�?�?中止(防被篡改/MITM 的二进制)
#     - 其它  当前是旧�?agent(不支�?__verify-update)或拿不到 .sig �?警告后继�?过渡期兼�?
SIG="$TMP.sig"
sig_ok=0
for URL in "${MIRRORS[@]}"; do
    if command -v curl >/dev/null 2>&1; then
...
if [ "$sig_ok" = 1 ] && [ -x "$BIN" ] && command -v timeout >/dev/null 2>&1; then
    ...
    if [ "$VRC" = 0 ]; then
        ...
        err "签名校验失败(二进制与签名不匹�?拒绝升级): $VOUT"
    else
        log "[WARN] 无法验签(rc=$VRC,可能当前为旧�?agent 不支�?,按原流程继续"
    fi
else
    log "[WARN] 未获取到 .sig 或环境不支持,跳过验签"
fi

# 4. 与现�?binary 对比;一样就不动
if [ -f "$BIN" ]; then
    OLD_MD5=$(md5sum "$BIN" | awk '{print $1}')
    if [ "$OLD_MD5" = "$NEW_MD5" ]; then
        log "现有 binary 已是该版�?(md5=$NEW_MD5),无需替换"
        exit 0
    fi
    BAK="${BIN}.bak-$(date +%s)"
    cp "$BIN" "$BAK"
    log "已备�? $BAK (md5=$OLD_MD5)"
fi
...
if [ -d /run/systemd/system ] && command -v systemctl >/dev/null 2>&1 \
   && systemctl list-unit-files mmw-agent.service >/dev/null 2>&1; then
    ...
    # �?nohup 模式 �?kill 老进�?�?binary 需要用户原命令再启
    log "[WARN] 检测到�?systemd/OpenRC 模式 mmw-agent 进程,本脚本不自动重启"
    log "[WARN] 请你手动:pkill -f /usr/local/bin/mmw-agent && nohup /usr/local/bin/mmw-agent -c <config> &"
else
    log "[WARN] 未检测到 mmw-agent 进程或服�?二进制已替换但需要手动启�?
fi
...
if [ $restarted -eq 1 ]; then
    if pgrep -f "/usr/local/bin/mmw-agent" >/dev/null 2>&1; then
        ...
        log "[ERROR] agent 进程未起�?查看 journalctl -u mmw-agent / /var/log/mmw-agent.log 排查"
        log "[ERROR] 回滚命令: mv $BAK $BIN && systemctl restart mmw-agent  # �?rc-service mmw-agent restart"
        exit 1
    fi
fi
