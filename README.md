...
妙妙屋X 远程服务器代理程序。部署在子服务器上，负责与主控（miaomiaowux）通信，上报流�?速度数据，并接受主控的远程管理指令�?

## 部署方式

### 方式 1:Docker(推荐)

镜像内置 embedded xray-core(�?Go �? + nginx + WSS / Reality 反代所需的全套依�?拉起即可用�?
...
docker run -d \
  ...
  -e MMWX_MASTER_TOKEN=<主控生成�?token> \
...
**强制约束**:
- **必须 host 网络模式**(`--network host` / `network_mode: host`)。xray 入站端口由用户在主控前端动态创�?bridge 模式得每开一个就�?`-p` 映射,�?agent 监听端口给主控反向连接也走宿主网络。容�?entrypoint 启动时检�?bridge 模式会直接退出报错�?
- **主控添加 server 时�?embedded 模式**。镜像里没有外部 xray binary,只有嵌入�?xray-core,主控必须�?embedded 协议对接�?
- 调试时绕�?host 检�?`-e MMWX_REQUIRE_HOST_NETWORK=0`(不推�?有概率端口冲�?�?

### 方式 2:二进�?+ systemd(传统裸机部署)

通过主控前端「添�?server」生成的一键脚本安�?流程跟原来一�?�?自动�?xray binary + �?systemd + 注册到主控�?
...
- 🌈修改用户网速的统计方式
- 🌈升级镜像nginx版本
- 🛠�?fix:vless reality出站缺少sniffing.exclude配置
...
### v0.3.0 (2026-06-14)
- 🛠�?fix:warp出站冗余配置
- 🛠�?fix:只有ipv6的机器无法下周geodata
- 🛠�?fix:增加客户端限制通知
- 🛠�?fix:断开连接后重连慢
- 🛠�?fix:用户的路由出站规则插入位置不�?

### v0.2.9 (2026-06-10)
- 🛠�?fix:偷自己nginx配置目录下发错误
...
�? /api/remote/traffic   HTTP 流量推�?                �?
�? /api/remote/speed     HTTP 速度推�?                �?
...
speed_report_interval: 3         # �?
heartbeat_interval: 30           # �?
xray_servers:                    # 可选，不配则自动发�?
...
| `MMWX_MASTER_TOKEN` | 服务器令�?| �?|
| `MMWX_CONNECTION_MODE` | 连接模式 | `auto` |
| `MMWX_LISTEN_PORT` | 监听端口 | `23889` |
| `MMWX_CHILD_API_TOKEN` | Pull API 认证令牌 | �?|
| `MMWX_TRAFFIC_INTERVAL` | 流量上报间隔(�? | `60` |
| `MMWX_SPEED_INTERVAL` | 速度上报间隔(�? | `3` |
| `MMWX_HEARTBEAT_INTERVAL` | 心跳间隔(�? | `30` |

环境变量优先级高于配置文件�?

### Xray 自动发现

未显式配�?`xray_servers` 时，agent 按以下路径搜�?Xray 配置�?
...
连续 5 次连接失败后自动切换�?Auto 模式回退�?

### HTTP

单向推送。Agent 定时 POST 数据�?Master�?
...
被动模式。Agent 仅暴露本�?API，等�?Master 主动拉取�?
...
未通过认证的连接被**静默丢弃**（TCP hijack 后直接关闭，不返�?HTTP 响应）�?
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/child/services/status` | 获取 Xray/Nginx 服务运行状�?|
...
// 并发 TCP 探测，自动去重，最�?200 个域�?
```

**扫描自动补全**：`/api/child/scan` 会检�?Xray 配置完整性，自动补全缺失�?`api`、`stats`、`policy` 段及 API 入站和路由规则，补全后自动重�?Xray�?
...
// 单位：字�?�?(B/s)，来源：/proc/net/dev 物理网卡聚合
```
...
| `domain_latency_result` | 域延迟探测结�?| `{ "request_id": "string", "success": bool, "results": [{ "domain": "string", "latency_ms": int64, "success": bool }] }` |
...
| `domain_latency_probe` | 域延迟探测请�?| `{ "request_id": "string", "domains": ["string"], "timeout_ms": int }` |

### 认证流程

1. Agent 建立 WebSocket 连接（必须携�?`User-Agent: miaomiaowux/0.1`�?
2. Agent 发�?`auth` 消息，携带服务器令牌
3. Master 验证令牌，返�?`auth_result`
4. 认证成功后进入消息循环；未认证的消息会收�?`auth_result { success: false, message: "Authentication required" }`
...
Agent 认证成功后自动执�?Xray 扫描，将结果通过 `scan_result` 发送给 Master。如果服务器处于 `pending` 状态且配置了域名和 443 端口，Master 会自动触�?steal-self 配置部署�?
...
�?Xray �?`/debug/vars` HTTP 端点采集，解�?`stats` 对象中的 inbound/outbound/user 流量数据�?

metrics 地址�?Xray 配置文件�?`api` 段中�?`listen` 字段获取，默�?`127.0.0.1:38889`�?
